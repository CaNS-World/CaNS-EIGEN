...
Wall velocities, forcing and bulk targets are read from the corresponding physical entries of `bcvel`, `bforce`, `is_forced` and `velf`; these inputs and the grid are not reordered. Existing boundary-condition restrictions still apply, including homogeneous velocity values on FFT axes with implicit diffusion.
