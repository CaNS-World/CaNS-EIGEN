...
module mod_bound
  ...
  contains
  subroutine bounduvw(cbc,n,bc,nb,is_bound,dxc,dxf,dyc,dyf,dzc,dzf,u,v,w, &
                      preserve_normal_values,normal_halos,tangential_halos)
    !
    ...
    if(is_bound(0,1)) then
      if(apply_normal_bc(0,1)) call set_bc(cbc(0,1,1),0,1,nh,.false.,bc(0,1,1),dxf(0),u)
      call set_bc(cbc(0,1,2),0,1,nh,.true.,bc(0,1,2),dxc(0),v)
      call set_bc(cbc(0,1,3),0,1,nh,.true.,bc(0,1,3),dxc(0),w)
    end if
    if(is_bound(1,1)) then
      if(apply_normal_bc(1,1)) call set_bc(cbc(1,1,1),1,1,nh,.false.,bc(1,1,1),dxf(n(1)),u)
      call set_bc(cbc(1,1,2),1,1,nh,.true.,bc(1,1,2),dxc(n(1)),v)
      call set_bc(cbc(1,1,3),1,1,nh,.true.,bc(1,1,3),dxc(n(1)),w)
    end if
    if(is_bound(0,2)) then
      call set_bc(cbc(0,2,1),0,2,nh,.true.,bc(0,2,1),dyc(0),u)
      if(apply_normal_bc(0,2)) call set_bc(cbc(0,2,2),0,2,nh,.false.,bc(0,2,2),dyf(0),v)
      call set_bc(cbc(0,2,3),0,2,nh,.true.,bc(0,2,3),dyc(0),w)
    end if
    if(is_bound(1,2)) then
      call set_bc(cbc(1,2,1),1,2,nh,.true.,bc(1,2,1),dyc(n(2)),u)
      if(apply_normal_bc(1,2)) call set_bc(cbc(1,2,2),1,2,nh,.false.,bc(1,2,2),dyf(n(2)),v)
      call set_bc(cbc(1,2,3),1,2,nh,.true.,bc(1,2,3),dyc(n(2)),w)
    end if
  ...
  !
  subroutine boundp(cbc,n,bc,nb,is_bound,dxc,dyc,dzc,p,halo_side)
    !
    ...
    if(is_bound(0,1).and.update_halo(0)) then
      call set_bc(cbc(0,1),0,1,nh,.true.,bc(0,1),dxc(0),p)
    end if
    if(is_bound(1,1).and.update_halo(1)) then
      call set_bc(cbc(1,1),1,1,nh,.true.,bc(1,1),dxc(n(1)),p)
    end if
    if(is_bound(0,2).and.update_halo(0)) then
      call set_bc(cbc(0,2),0,2,nh,.true.,bc(0,2),dyc(0),p)
    end if
    if(is_bound(1,2).and.update_halo(1)) then
      call set_bc(cbc(1,2),1,2,nh,.true.,bc(1,2),dyc(n(2)),p)
    end if
