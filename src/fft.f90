...
module mod_fft
  ...
  subroutine fftini(ng,is_fft,n_x,n_y,bcxy,c_or_f,arrplan,normfft)
    ...
    !
#if !defined(_OPENACC)
    if(any(is_fft).and.nplans == 0) then
    ...
    !
    if((any(is_fft.and.(c_or_f(:) == 'f').and.(bcxy(0,:) /= bcxy(1,:)))).and.(.not.allocated(fft_work))) then
...
#endif
    normfft = 1.
    if(is_fft(1)) call fftini_axis(1,ng(1),n_x,bcxy(:,1),c_or_f(1),arrplan(:,1),normfft(1))
    if(is_fft(2)) call fftini_axis(2,ng(2),n_y,bcxy(:,2),c_or_f(2),arrplan(:,2),normfft(2))
#if !defined(_OPENACC)
    do j=1,2
  ...
  subroutine prep_dctviii(f_or_b,cbc,idir,arr,arr_out)
    ...
    select case(idir)
    case(1)
      !$OMP PARALLEL DO COLLAPSE(3) DEFAULT(shared) PRIVATE(ii)
      do k=1,n(3)
        do j=1,n(2)
          do i=1,m
            if(f_or_b == 'F') then
    ...
    case(2)
      !$OMP PARALLEL DO COLLAPSE(3) DEFAULT(shared) PRIVATE(jj)
      do k=1,n(3)
        do j=1,m
          do i=1,n(1)
            if(f_or_b == 'F') then
