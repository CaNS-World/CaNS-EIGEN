...
module mod_post
  ...
  subroutine vorticity(n,dxci,dyci,dzci,ux,uy,uz,vox,voy,voz)
    ...
    do k=0,n(3)
      do j=0,n(2)
        do i=1,n(1)
          vox(i,j,k) = (uz(i,j+1,k)-uz(i,j,k))*dyci(j) - &
    ...
    do k=1,n(3)
      do j=0,n(2)
        do i=0,n(1)
          voz(i,j,k) = (uy(i+1,j,k)-uy(i,j,k))*dxci(i) - &
                       (ux(i,j+1,k)-ux(i,j,k))*dyci(j)
        end do
      end do
    end do
  ...
  subroutine strain_rate(n,dxci,dxfi,dyci,dyfi,dzci,dzfi,ux,uy,uz,str)
    ...
    do k=1,n(3)
      do j=0,n(2)
        do i=0,n(1)
          s12e(i,j,k) = .5_rp*((ux(i,j+1,k)-ux(i,j,k))*dyci(j) + &
                               (uy(i+1,j,k)-uy(i,j,k))*dxci(i))
        end do
      end do
    end do
    !$acc parallel     loop collapse(3) default(present)
