...
module mod_solver_gpu
  ...
  subroutine solver_gpu(n,ng,is_fft,arrplan,normfft,lambdaxy,eigvecx_fwd,eigvecx_bwd,eigvecy_fwd,eigvecy_bwd,a,b,c, &
                        bc,c_or_f,p,is_dtdma_update,aa_z,cc_z)
    implicit none
    ...
    px_1(1:n_x(1),1:n_x(2),1:n_x(3)) => solver_buf_0(1:product(int(n_x(:),i8)))
    px_2(1:n_x(1),1:n_x(2),1:n_x(3)) => solver_buf_0(1:product(int(n_x(:),i8)))
    if(cudecomp_is_t_in_place.and.is_fft(2)) then ! no in-place allowed with gemm along y
      ...
      py_1(1:n_y(1),1:n_y(2),1:n_y(3)) => solver_buf_0(1:product(int(n_y(:),i8)))
      py_2(1:n_y(1),1:n_y(2),1:n_y(3)) => solver_buf_0(1:product(int(n_y(:),i8)))
    else
      py(  1:n_y(1),1:n_y(2),1:n_y(3)) => solver_buf_1(1:product(int(n_y(:),i8)))
      py_1(1:n_y(1),1:n_y(2),1:n_y(3)) => solver_buf_1(1:product(int(n_y(:),i8)))
      py_2(1:n_y(1),1:n_y(2),1:n_y(3)) => solver_buf_1(1:product(int(n_y(:),i8)))
    end if
    pz(1:n_z(1),1:n_z(2),1:n_z(3)) => solver_buf_0(1:product(int(n_z(:),i8)))
    if(.not.is_fft(1)) then
      px_1(1:n_x(1),1:n_x(2),1:n_x(3)) => solver_buf_0(1:product(int(n_x(:),i8)))
      px_2(1:n_x(1),1:n_x(2),1:n_x(3)) => gemm_buf_x(  1:product(int(n_x(:),i8)))
    end if
    if(.not.is_fft(2)) then
      py_1(1:n_y(1),1:n_y(2),1:n_y(3)) => solver_buf_1(1:product(int(n_y(:),i8)))
      py_2(1:n_y(1),1:n_y(2),1:n_y(3)) => gemm_buf_y(  1:product(int(n_y(:),i8)))
    end if
    if(is_fft(1)) then
  ...
  subroutine solver_gaussel_yz_gpu(n,ng,is_fft,arrplan,normfft,lambday,eigvecy_fwd,eigvecy_bwd,a,b,c,bc,c_or_f,p)
    ...
    pz(1:n(1),1:n(2),1:n(3)) => solver_buf_0(1:product(int(n(:),i8)))
    if(is_fft(2)) then
      call fft_gpu_layout(ng(2),n_y,bc(0,2)//bc(1,2),c_or_f(2),nfft,nwork)
      pfft_tmp_y(1:nwork(1),1:nwork(2),1:nwork(3)) => work(1:product(int(nwork(:),i8)))
    end if
    ...
    !
    q = merge(1,0,c_or_f(3) == 'f'.and.bc(1,3) /= 'P'.and.n(3) == ng(3))
    is_periodic_z = bc(0,3)//bc(1,3) == 'PP'
