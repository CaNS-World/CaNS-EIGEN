...
def norms(error, weights):
    return np.array([(np.sqrt(np.sum(e*e*w)/np.sum(w)), np.max(abs(e))) for e, w in zip(error, weights)])
...
def run(n, steps, axis):
    ...
    geometry = [np.fromfile(case/'data'/f'grid_{direction}.bin', dtype=np.float64).reshape(4, size)
                for direction, size in zip('xyz', grid)]
    weights = np.array([geometry[a][0, :, None]*geometry[b][1, None, :],
                        geometry[a][1, :, None]*geometry[b][0, None, :]])
    exact = np.array([np.cos(geometry[a][3, :, None])*np.sin(geometry[b][2, None, :]),
                      -np.sin(geometry[a][2, :, None])*np.cos(geometry[b][3, None, :])])
    return np.array([f.transpose(a, b, axis)[:, :, 0] for f in (fields[a], fields[b])]), time, exact, weights
...
def main():
    ...
    for axis in range(3):
        ...
        errors = []
        for n in (16, 32, 64):
            uv, time, exact, weights = run(n, 100, axis)
            error = norms(uv-exact*np.exp(-.2*time), weights)  # nu=0.1, T=1
            errors.append(error)
        ...
        temporal = [run(32, steps, axis) for steps in (20, 40, 80, 160)]
        solutions = [result[0] for result in temporal]
        weights = temporal[0][3]
        differences = [norms(a-b, weights) for a, b in zip(solutions, solutions[1:])]
